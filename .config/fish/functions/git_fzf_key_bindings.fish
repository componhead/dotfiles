...
function __git_fzf_git_status
	...
	git -c color.status=always status --short | \
		fzf-layout -m --ansi --preview 'git diff --color=always HEAD -- {-1} | head -500' | \
		cut -c4- | \
		sed 's/.* -> //' | \
		tr '\n' '*' | \
		sed 's/\*$//' | \
...
function __git_fzf_git_branch
	...
	git branch -a --color=always | \
		grep -v '/HEAD\s' | \
		fzf-layout -m --ansi --preview-window right:70% --preview 'git log --color=always --oneline --graph --date=short \
...
function __git_fzf_git_tag
	...
	git tag --sort -version:refname | \
		fzf-layout --ansi --preview-window right:70% \
...
function __git_fzf_git_log
	...
	git log --color=always --graph --date=short --format="%C(auto)%cd %h%d %s %C(magenta)[%an]%Creset" | \
		fzf-layout -m --ansi --reverse --preview 'git show --color=always (echo {} | grep -o "[a-f0-9]\{7,\}") | head -'$LINES | \
		awk '{print $3}' \
...
function git_fzf_key_bindings -d "Set custom key bindings for git+fzf"
	...
	bind -M insert -m insert \cg\cr __git_fzf_git_remote
end
